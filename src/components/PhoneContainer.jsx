...
const PhoneContainer = ({ phones }) => {
  ...
  //   1.5 use useEffect for rendering
  useEffect(() => {
    // 1.6 now render the phones data upon showAll condition. if showAll is true all the phones will be shown
    if (showAll) {
      setDisplayPhones(phones);

      //   1.7 if the showAll false then show only 6 phones
    } else {
    ...
    <div>
      ...
      >
        <span className="relative z-10 block px-5 py-3 overflow-hidden font-medium leading-tight text-gray-800 transition-colors duration-300 ease-out border-2 border-gray-900 rounded-lg group-hover:text-white">
          ...
          {/* 1.9 changing the button text show All to showLess and showLess to showAll upon clicking*/}
